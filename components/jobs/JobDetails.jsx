...
import { useGetJobDetailsMutation } from "../Features/jobs/jobsApiSlice";
...
export default JobDetails = ({ route }) => {
  ...
  const [getJobDetails, { isLoading }] = useGetJobDetailsMutation();
  useEffect(() => {
    const getJobs = async () => {
      const data = await getJobDetails({ id: id }).unwrap();
      data && setData(data);
    };
    getJobs();
  }, []);
  return isLoading ? (
    ...
    >
      ...
      >
        ...
        >
          <View>
            <Text style={[styles.fonSiz18, styles.fonWei700]}>Job details</Text>
          </View>
